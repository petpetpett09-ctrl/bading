<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="max-w-3xl mx-auto">
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-4xl font-bold text-slate-900 mb-2">Create New Invoice</h1>
        <p class="text-slate-600">Record a new customer invoice</p>
      </div>
      ...
      <form @submit.prevent="submit" class="bg-white rounded-lg shadow p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <input v-model="form.invoice_number" type="text" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>

          <!-- Customer -->
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-2">Customer *</label>
            <input v-model="form.customer" type="text" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>

          <!-- Issue Date -->
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-2">Issue Date *</label>
            <input v-model="form.issue_date" type="date" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
        ...
        <div class="flex gap-4">
          <button type="submit" :disabled="form.processing" class="flex-1 bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition font-medium">
            {{ form.processing ? 'Processing...' : 'Create Invoice' }}
          </button>
